...
def infer_fast(net, img, net_input_height_size, stride, upsample_ratio, cpu,
    ...
    #print("shape:",img.shape)#shap: (850, 1514, 3)  shap: (455, 3)
    ...
    #print("min_dims",min_dims)#min_dims [256, 456]
    padded_img, pad = pad_width(scaled_img, stride, pad_value, min_dims)  # 填充到高宽为stride 整数倍的值
    #print("padded_img",padded_img)#padded_img [[[  -0.085938    0.015625    0.027344]
    #print(padded_img.shape)#(256, 456, 3)

    tensor_img = from_numpy(padded_img).permute(2, 0, 1).unsqueeze(0).float()  # 有HWC转成CHW(BGR格式)
    #print("tensor_img",tensor_img)#tensor_img tensor([[[[-0.08594, -0.08594, -0.08984,  ..., -0.19531, -0.19531, -0.18359],
    if not cpu:
...
def run_demo(net, action_net, img, height_size, cpu, boxList, vedio_flag):
    net = net.eval()
    if not cpu:
        net = net.cuda()
    ...
    if i % 1 == 0 :
        ...
        for n in range(len(pose_entries)):  # 依次遍历找到的每个人
            ...
            if boxList:
                ...
                if len(pose.getKeyPoints()) >= 10 and coincideValue >= 0.3 :
        ...
        for pose in current_poses:
            ...
            cv2.rectangle(img, (pose.bbox[0], pose.bbox[1]),
                              (pose.bbox[0] + pose.bbox[2], pose.bbox[1] + pose.bbox[3]), (255, 0, 0), thickness=1)
            cv2.putText(img, '{}'.format(pose.pose_action), (pose.bbox[0], pose.bbox[1] - 16),
                            cv2.FONT_HERSHEY_COMPLEX, 0.7, (255, 255, 255),thickness=2)
            #fallFlag = 1
            # else:
            #     cv2.rectangle(img, (pose.bbox[0], pose.bbox[1]),
...
def detect_main(video_name=''):
    ...
    parser.add_argument('--images', nargs='+',
                        default='E:\\ism_person_openpose\\data\\pics',
    ...
    run_demo(net, action_net, frame_provider, args.height_size, False, [])
